CREATE PROCEDURE [dbo].[BackupStatusReportSuccess]
AS
...
FROM msdb..backupset e
              WHERE e.server_name = @@SERVERNAME  AND e.database_name NOT IN ('tempdb') AND 
                                                  e.database_name IN (SELECT Distinct name FROM master.sys.databases where database_id<>2 and is_read_only <> 1 and state_desc <> 'OFFLINE')
)
