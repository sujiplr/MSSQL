CREATE PROCEDURE [dbo].[BackupCount]
AS
...
SET NOCOUNT ON
	...
	DECLARE @table2 table (Servername nvarchar(60),  DBName nvarchar(60),
...
select distinct bkp_t1.Servername,bkp_t1.DBname, (SELECT  CONVERT(varchar(25),MAX(backup_finish_date) , 100)
		...
		WHILE @@FETCH_STATUS=0
			...
			BEGIN
			   ...
			   IF (@LastDiffBackup IS NULL)
				...
				INSERT INTO @table2 values (@servername,@dbname,@LastFullBackup,@LastDiffBackup,@FinalAge,@status)

				FETCH NEXT FROM c1 INTO @servername,@dbname,@LastFullBackup,@LastDiffBackup,@NotBackedupSinceHrs,@status

			END

	UPDATE @table2 SET status = CASE
		WHEN NotBackedUpSince <=@check THEN 'Success'
		WHEN NotBackedUpSince > = @check THEN 'Failed/NotRun'        
	END

	UPDATE @table2 SET Status='Success' 
	...
	DECLARE @a1 int
	DECLARE @b1 int
	DECLARE @table3 table (ServerName varchar(50),Success int,Failed int)
 	SELECT @a1 = count (*) FROM @table2 WHERE Status = 'Success'	
	SELECT @b1= count (*) FROM @table2 WHERE Status like '%Failed%'
	INSERT INTO @table3 
	SELECT @@SERVERNAME,@a1,@b1
